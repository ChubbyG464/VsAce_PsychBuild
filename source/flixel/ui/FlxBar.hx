...
class FlxBar extends FlxSprite
{
	...
	static inline var _maxPercent:Int = 100;
	 ...
	 */
	public inline function stopTrackingParent(posX:Int, posY:Int):Void
	{
		fixedPosition = true;
		x = posX;
		y = posY;
	...
	override public function update(elapsed:Float):Void
	{
		if (parent != null)
		{
			var parentRef = Reflect.getProperty(parent, parentVariable);
			if (parentRef != value)
			{
				value = parentRef;
				//updateValueFromParent();
	...
	/**
	 * Internal function for setting property for this object.
	 * It changes graphics' `useCount` also for better memory tracking.
	 */
	function changeFrame<T:FlxFramesCollection>(Value:T, Old:FlxFramesCollection)
	{
		var oldGraphic:FlxFramesCollection = Old;

		if ((Old != Value) && (Value != null))
			Value.parent.useCount++;

		if ((oldGraphic != null) && (oldGraphic != Value))
			oldGraphic.parent.useCount--;
	...
	function set_frontFrames(value:FlxImageFrame):FlxImageFrame
	{
		if (FlxG.renderTile)
		{
			frontFrames = changeFrame(value, frontFrames);
	...
	function set_backFrames(value:FlxImageFrame):FlxImageFrame
	{
		if (FlxG.renderTile)
		{
			frames = changeFrame(value, frames);
		}
		else
		{
